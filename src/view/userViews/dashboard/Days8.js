...
const Days8 = ({ navigation, geoLocation, headerArrowPressed, selectedLocation, InternetConnected, allLocations, indexTime, isDay }) => {

    const huntingDetails = useSelector(state => state.hunt.huntingDetails)
    const [isModalVisible, setModalVisible] = useState(false);
    const [huntingFor8Day, setHuntingFor8Day] = useState({})
    const [fetching, setFetching] = useState(false)
    useEffect(() => {
        setFetching(true)
        if (Object.keys(huntingDetails).length !== 0) {
            setHuntingFor8Day(huntingDetails[`${geoLocation?.city},${geoLocation?.state}`]?.slice(6, 10) || {})
        }
        setTimeout(() => {
            setFetching(false)
        }, 1500);
    ...
    const handlePressIcon = (iconType) => {
        ...
        else if (iconType === Date_Arrow_Icon_Type.RIGHT) {
            navigation.navigate(RouteNames.Dashboard['12-Days'])
        }
    }
    ...
    const RenderWeatherBox = ({ item, index, toggleModal }) => {
        // let dayFlag = item?.weather?.length && item?.weather_day_night[0]?.isDay
        return (
    ...
    if (selectedLocation && Object.values(huntingFor8Day)?.length === 0 && allLocations.length && !headerArrowPressed && !fetching) {
        return (
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
    ...
    const toggleModal = (index) => {
        if (huntingFor8Day[index]?.weather) {
            setIndex(index)
    ...
    // let dayFlag = huntingFor8Day[0]?.weather && huntingFor8Day[0]?.weather_day_night[0]?.isDay
    return (
        huntingFor8Day && huntingFor8Day.length && !headerArrowPressed && selectedLocation ?
            <ScrollView showsVerticalScrollIndicator={false} bounces={false} style={{ flex: 1, backgroundColor: primaryColor }}>
                <View style={{ flex: 1.7, backgroundColor: 'white' }}>
                    <View style={{ flex: 1 }}>
                        <DateComponent onPressIcon={handlePressIcon} date={huntingFor8Day[0]?.original_date} />
                    </View>
                    <View style={{ flex: 1, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', paddingHorizontal: normalizeWidth(5) }}>
                        {
                            huntingFor8Day.map((each) => {
                                return <RenderRoutineBox item={each} key={each.original_date} />
                            })
                        }
                    </View>
                    <DashboardModal handleModal={toggleModal} isModalVisible={isModalVisible} item={huntingFor8Day[index]} />
                ...
                <View style={{ flex: 1, flexDirection: 'row', backgroundColor: primaryColor, height: normalizeHeight(270), paddingHorizontal: normalizeWidth(15) }}>
                    <View style={{ flex: 1.2, justifyContent: 'space-evenly', paddingBottom: normalizeHeight(60) }}>
                        <View >
                            <Text style={styles.moonDetailText}>Moon Phase: {huntingFor8Day[0]?.moon_phase_illumination}</Text>
                            <Text style={styles.moonDetailText}>Moon Rise: {huntingFor8Day[0]?.moon_rise}</Text>
                            <Text style={styles.moonDetailText}>Moon Set: {huntingFor8Day[0]?.moon_set}</Text>
                            <Text style={styles.moonDetailText}>Moon Transit: {huntingFor8Day[0]?.moon_transit_api}</Text>
                        </View>
                        <View>
                            ...
                            <View style={{
                                ...
                                <View style={{ flexDirection: "column", }} >
                                    ...
                                    <Text style={styles.moonDetailText} >{`(inHg) ${huntingFor8Day[0]?.weather?.pressureIN} ${huntingFor8Day[0]?.weather?.weather}`}</Text>
                                </View>
                    ...
                    <View style={{ flex: 1, alignItems: 'flex-end', justifyContent: 'center', paddingBottom: normalizeHeight(60) }}>
                        <Text style={styles.moonText}>{huntingFor8Day[0]?.moon_phase_name}</Text>
                        <Image source={{ uri: huntingFor8Day[0]?.moon_image }} style={{ width: '70%', height: '70%' }} resizeMode="contain" />
                    </View>
                </View>
            </ScrollView >
            :
...
export default connect(mapStateToProps, mapDispatchToProps)(Days8)
